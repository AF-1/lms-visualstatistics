...
sub getDataTrackTitleMostFrequentWords {
	...
	while ($sth->fetch()) {
		push (@alltitles, $thisTitle) unless ($thisTitle eq '');
	}

	my @ignoreCommonWords = ("able", "about", "above", "act", "adagio", "after", "again", "against", "ago", "ain", "akt", "album", "all", "also", "alt", "alternate", "always", "among", "and", "another", "any", "are", "aria", "around", "atto", "autre", "away", "back", "bad", "been", "before", "behind", "big", "black", "blue", "bonus", "but", "bwv", "can", "chanson", "che", "come", "comme", "con", "concerto", "cosa", "could", "dans", "das", "day", "del", "demo", "dein", "den", "der", "des", "did", "die", "don", "done", "down", "dub", "dur", "each", "edit", "ein", "either", "else", "est", "even", "ever", "every", "extended", "feat", "featuring", "first", "flat", "for", "from", "fur", "get", "girl", "gone", "gonna", "good", "got", "had", "has", "have", "heart", "her", "here", "him", "his", "home", "how", "ich", "iii", "instrumental", "interlude", "intro", "ist", "just", "keep", "know", "las", "les", "let", "life", "like", "little", "live", "long", "los", "major", "make", "man", "master", "may", "medley", "mein", "meu", "mind", "mine", "minor", "miss", "mix", "moderato", "moi", "moll", "molto", "mon", "mono", "more", "most", "much", "music", "must", "nao", "near", "need", "never", "new", "nicht", "non", "not", "now", "off", "old", "once", "one", "only", "orchestra", "original", "ouh", "our", "ours", "out", "over", "own", "part", "pas", "piano", "plus", "por", "pour", "prelude", "presto", "quartet", "que", "qui", "quite", "radio", "rather", "recitativo", "recorded", "remix", "right", "rock", "roll", "sao", "say", "scene", "see", "seem", "session", "she", "side", "single", "skit", "solo", "some", "something", "somos", "son", "sonata", "song", "sous", "stereo", "still", "street", "such", "suite", "symphony", "take", "tel", "tempo", "than", "that", "the", "their", "them", "then", "there", "these", "they", "thing", "think", "this", "those", "though", "thought", "three", "through", "thus", "time", "together", "too", "track", "trio", "try", "two", "una", "und", "under", "une", "until", "use", "version", "very", "vivace", "vocal", "wanna", "want", "was", "way", "well", "went", "were", "what", "when", "where", "whether", "which", "while", "who", "whose", "why", "will", "with", "world", "yet", "you", "your");
	sub regex {
		my ($word, @ignoreCommonWords) = @_;
		my $match = 0;
		my $re = join '|', map { quotemeta } @ignoreCommonWords;
		if ($word =~ /^($re)/) {
			$match = 1;
		}
		return $match;
	...
	foreach my $line (@alltitles) {
		my @words = split /\W+/, $line; #skip non-word characters
		foreach my $word(@words){
			chomp $word;
			$word = lc $word;
			$word =~ s/^\s+|\s+$//g; #remove beginning/trailing whitespace
			if ((length $word < 3) || (regex ($word, @ignoreCommonWords) == 1)) {next;}
			my $key = $word;
	...
	my @keys = ();
	foreach my $word (sort { $frequentwords{$a} <=> $frequentwords{$b} or "\F$a" cmp "\F$b"} keys %frequentwords) {
